...
void AesStreamCipher::initializeFromHexStr(
  ...
  if (keyDecStr.size() < CryptoPP::AES::MIN_KEYLENGTH)
  {
    ...
    sstr << "AesStreamCipher, key too small (min is: " << CryptoPP::AES::MIN_KEYLENGTH << ", input was " << keyDecStr.size() << ")";
  ...
  {
    std::stringstream sstr;
    sstr << "AesStreamCipher, key too big (max is: " << CryptoPP::AES::MAX_KEYLENGTH << ", input was " << keyDecStr.size() << ")";
    std::string msg = sstr.str();
    std::cerr << msg << std::endl;
    throw std::invalid_argument(msg);
  }
  ...
  {
    std::stringstream sstr;
    sstr << "AesStreamCipher, iv too small (min is: " << _encryption.MinIVLength() << ", input was " << ivDecStr.size() << ")";
    std::string msg = sstr.str();
    std::cerr << msg << std::endl;
    throw std::invalid_argument(msg);
  }
  ...
  {
    std::stringstream sstr;
    sstr << "AesStreamCipher, iv too big (max is: " << _encryption.MaxIVLength() << ", input was " << keyDecStr.size() << ")";
    std::string msg = sstr.str();
    std::cerr << msg << std::endl;
    throw std::invalid_argument(msg);
  }
...
std::string AesStreamCipher::encryptFromHexStrAsHexStr(const std::string& inHexStr)
{
...
std::string AesStreamCipher::decryptFromHexStrAsHexStr(const std::string& inHexStr)
{
  std::string dataDecStr = helpers::hexStr_to_byteBuffer(inHexStr);
  const uint8_t* dataDecStrPtr = reinterpret_cast<const uint8_t*>(dataDecStr.data());
