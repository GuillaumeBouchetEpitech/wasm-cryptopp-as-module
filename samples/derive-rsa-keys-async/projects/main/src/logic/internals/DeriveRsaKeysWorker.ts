...
import {
  ...
  IMsgDeriveRsaKeys_request,
  IMsgDeriveRsaKeys_response,
} from "../../../../_common"
...
const makeDeriveRsaKeysRequest = async (inWorker: Worker, id: string, password: string, keySize: number) =>
  _requestWorkerOnce<IMsgDeriveRsaKeys_request, IMsgDeriveRsaKeys_response>(inWorker, {
    type: StrategiesTypes.derive_rsa_keys, id, password, keySize
  });
  ...
  verifyHexStrPayloadToStr(signedHexStrPayload: string): string {
    ...
    try {
      ...
      if (typeof(err) === 'number') {
        throw new Error(wasmModule.getExceptionMessage(err));
      }
      throw err;
    }
  }
